...
* **`publish_image_file(file_path: str, description: str, ddx_uid: str)`**
  ...
  * *Usage Prompt:* "Publish the local image at './my-image.png' as a T-Doc."
...
* **`get_my_recent_published_messages()`**
  * *What it does:* Fetches the most recently sent T-Doc messages securely from the API using authentication JWT headers.
  * *Usage Prompt:* "Retrieve my recently published messages."
* **`get_user_recd_documents()`**
  * *What it does:* Fetches all T-Doc documents/messages where the authorized user is the recipient (`tgt` field matches the user's UID).
  * *Usage Prompt:* "Load and list all recent documents where I am the recipient."
* **`get_logged_in_user_ddxes(uid: str, optional)`**
  * *What it does:* Fetches all valid active DDX credentials and entitlements currently assigned to the logged-in agent (or a specified UID). Call this before `create_and_publish_tdoc` when you need a `ddx_uid`.
